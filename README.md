...
`ruby-test-services.yml` is the service-backed variant of `ruby-test.yml` for the service-backed gems and the service wave: opt-in boolean inputs add `postgres` (cimg/postgres:17.9, `POSTGRES_USER=fatsoma`), `rabbitmq` (4.2.2-alpine, broker only — for the prescribed `fatsoma_test` topology use `ruby-integration-rabbitmq.yml`), `redis` (3-alpine) and `elasticsearch` (5.6.10-alpine) service containers, each with a health check replacing CircleCI's `dockerize -wait`. A `db-setup` input (`structure-load` | `schema-load` | `test-prepare`) runs the matching Rails rake tasks before specs — contract §5 item 5 makes this an input, not a convention, because analytics/order (`db:test:prepare`) and ui-core (`db:schema:load`) differ from the pack. Artifacts are the same `test-results` / `coverage` pair as `ruby-test.yml`.
